...
def parse_effects_in_frame(line: str) -> List[str]:
    return [x[4:] for x in line.split('&br;') if x not in {'', '-'}]


def parse_effects_not_in_frame(line: str) -> List[str]:
    return [x for x in line.split('&br;') if x not in {'', '-'}]
...
def parse(line: str) -> CreatedItem:
    ...
    print(parse_name(part[0]), len(part))
    print(json.dumps({
        i: x
...
def main():
    raw = [x for x in open('p13_all_list.txt').read().split('\n') if x]
